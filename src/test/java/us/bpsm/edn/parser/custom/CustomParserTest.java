...
public class CustomParserTest {
	...
	public void testMapParser() {
		...
		HashMap<Keyword,CustomParser<Double>> pointMap=new HashMap<Keyword,CustomParser<Double>>();
		pointMap.put(kx,doubleParser);
		pointMap.put(ky,doubleParser);
		CustomParser<Point2D.Double> pointParser=new CustomParsers.MapParser<Point2D.Double>(pointMap) {
			@Override
		...
		HashMap<Keyword,CustomParser<Point2D.Double>> rectMap=new HashMap<Keyword,CustomParser<Point2D.Double>>();
		rectMap.put(kul,pointParser);
		rectMap.put(klr,pointParser);
		CustomParser<Rectangle2D.Double> rectParser=new CustomParsers.MapParser<Rectangle2D.Double>(rectMap) {
			@Override
			public Rectangle2D.Double construct(Map<?, ?> data) {
				...
				Rectangle2D.Double rect= new Rectangle2D.Double(ul.x,ul.y,0,0);
				rect.add(lr);
				return rect;
			}	 
		};
		
        Parseable r = Parsers.newParseable("{:upperLeft {:x 1.0 :y -1.0} :lowerRight {:x 5.0 :y 4}}");
